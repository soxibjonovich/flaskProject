...
@app.route('/<user_id>/<hash_id>/unarchive')
@login_required
async def unarchive(user_id, hash_id):
...
@login_required
async def activity_history_channel():
    activities = Tasks.query.filter_by(user_id=current_user.id, task_type=TaskType.ChannelDate).all()
    if request.method == "POST":
        ...

    return render_template('dashboard/history/channel_history.html', activities=activities)


@app.route('/history/group')
@login_required
async def activity_history_group():
    activities = Tasks.query.filter_by(user_id=current_user.id, task_type=TaskType.GroupToGroupDate).all()
    if request.method == "POST":
        ...

    return render_template('dashboard/history/group_history.html', activities=activities)


@app.route('/history/private')
@login_required
async def activity_history_private():
    activities = Tasks.query.filter_by(user_id=current_user.id, task_type=TaskType.PrivateChat).all()
    if request.method == "POST":
        ...

    return render_template('dashboard/history/private_history.html', activities=activities)
...
async def delete_task():
    ...
    tasks = Tasks.query.filter_by(task_author=author, link=link).all()
    for task in tasks:
    ...
    if task_type == "group":
        return redirect(url_for('activity_history_group'))
    elif task_type == "channel":
        return redirect(url_for('activity_history_channel'))
...
if __name__ == '__main__':
    async def check_tasks():
        ...
        with app.app_context():
            print("[*] checking")
            tasks_to_delete = Tasks.query.filter(Tasks.date <= now).all()
